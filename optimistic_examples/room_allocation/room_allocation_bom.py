...
from optimistic_client.meta.utils import count
...
class Employee(Resource):
    type: EmployeeType
    number: int = field(metadata=dict(key=True))
    team_lead: 'Employee'
    is_team_lead: bool
    is_independent: bool
    area: Area
...
class RoomAllocationProblem(ResourceAllocationProblem):
    ...
    def constraint2(self) -> bool:
        return all(self.unique_assignment(e1) == self.unique_assignment(e1.team_lead)
                   for e1 in self.employees if not e1.team_lead.is_independent)
    ...
    def cost_penalty(self, area: Area, f1: Floor) -> float:
        return -1.1 if self.area_utilization(area, f1) == 0 else 1.1
    ...
    def objective(self) -> float:
        return 1 * self.objective1() + 1 * self.objective2()
